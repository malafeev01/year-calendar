...
function getEventsForDate (allEvents, date) {
  let events = []
...
function getDisabledHours(event, isStartTime) {
  let endHour = isStartTime ? moment(event.end.dateTime).hour() :
                              moment(event.start.dateTime).hour()
  let disabledHours = []
  for (let i=0; i<24;i++){
    if (isStartTime) {
...
function getDisabledMinutes(event, isStartTime) {
  let endMinute = isStartTime ? moment(event.end.dateTime).minute() :
                                moment(event.start.dateTime).minute()
  let disabledMinutes = []
  for (let i=0; i<60;i++){
    if (isStartTime) {
